...
const QuestionOptions = ({ question }: IProps) => {
	...
	return (
		<div className='flex items-center justify-around px-3 mt-1'>
			<button
				onClick={() => previewModal.openModal(question)}
				className='hover:bg-slate-100/20 p-1.5 rounded-md'
			>
				<svg
					className='w-5'
					fill='currentColor'
					...
					toast.custom(t => (
						<DecitionToast
							...
							afirmativeCallback={async () => {
								try {
									const res = await deleteQuestionRequest(
										question._id,
										auth.token
									)
									...
									if (
										res.status === 200 &&
										res.data.message === "Question deleted"
									) {
...
const QuestionCard = ({ question, index }: IProps) => {
	...
	const isHovered = useHoverDirty(questionCardRef)
	...
	return (
		...
		>
			...
			<div className={`${isHovered ? "max-h-[200px]" : "max-h-0"} transition-all duration-700 overflow-hidden`}>
