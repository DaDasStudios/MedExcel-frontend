...
const DeleteAccountScreen = () => {
	...
	async function deleteAccount() {
		try {
			const permissionToken = new URLSearchParams(location.search).get("permissionToken")
			const { data } = await deleteAccountAuthorized(permissionToken || "")
					...
					<article className='mx-auto max-w-lg'>
						<div className='py-6 px-5 sm:p-8 bg-slate-900/50 rounded-md border border-slate-100/10 flex flex-col'>
							<div className='mb-6 text-center flex flex-col justify-center gap-3'>
								<h1 className='text-4xl font-bold text-slate-100 tracking-tight'>Delete account</h1>
								<p className='text-slate-300'>
									Are you sure you want to delete your account? This action is not reversible anyway
								...
								<div className='flex items-center justify-center mt-3'>
									...
									>
										<span className='flex gap-2'>
											...
											<p className='sm:block hidden'>I do want to delete my account</p>
											<p className='sm:hidden block'>Confirm deletion</p>
										</span>
									</SolidButton>
								</div>
							</div>
